...
class TestCompanyPortal:
    ...
    def test_company_login_success(self, client):
        Company.objects.create(
            name="Empresa Portal", email="portal@empresa.com", cnpj="12.345.678/0001-99"
        )
        url = reverse("companies:company-login")
        payload = {"cnpj": "12.345.678/0001-99", "email": "portal@empresa.com"}
    ...
    def test_company_login_not_found(self, client):
        url = reverse("companies:company-login")
        payload = {"cnpj": "00.000.000/0000-00", "email": "inexistente@empresa.com"}
        response = client.post(url, data=payload, content_type="application/json")
    ...
    def test_update_job_permissions(self, client):
        ...
        response = client.put(
            url, data=payload, content_type="application/json", **headers_b
        )
        ...
        response = client.put(
            url, data=payload, content_type="application/json", **headers_a
        )
        assert response.status_code == 200
    ...
    def test_list_my_jobs(self, client):
        company1 = Company.objects.create(
            name="Empresa 1", email="1@1.com", cnpj="11.111.111/0001-11"
        )
        company2 = Company.objects.create(
            name="Empresa 2", email="2@2.com", cnpj="22.222.222/0001-22"
        )
        ...
        Job.objects.create(
            company=company1, title="Vaga 1B", description="D", is_active=True
        )
        Job.objects.create(
            company=company2, title="Vaga 2A", description="D", is_active=True
        )
