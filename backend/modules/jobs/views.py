...
class JobListAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not company:
            return Response({"detail": "Não autorizado. Faça login como empresa."}, status=status.HTTP_401_UNAUTHORIZED)
...
class JobDetailAPIView(APIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        ...
        if job.company != company:
            return Response({"detail": "Permissão negada. Esta vaga não pertence à sua empresa."}, status=status.HTTP_403_FORBIDDEN)
...
class CompanyJobsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = Job.objects.filter(company=company).exclude(status="deleted").order_by("-created_at")
            ...
            if recipient_email:
                try:
                    ...
                    email = EmailMessage(
                        subject=subject,
                        body=body,
                        from_email=config("DEFAULT_FROM_EMAIL", default="noreply@jacarei.sp.gov.br"),
                        to=[recipient_email]
                    )
