...
type SourceType = "all" | "local" | "indeed" | "vagas" | "adzuna";
...
export default function Home() {
  ...
  const loadJobs = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetchJobs(page, debouncedSearch, undefined, PAGE_SIZE);
      ...
      <Container maxWidth="lg" sx={{ flex: 1, py: 5, px: { xs: 2, md: 3 } }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", mb: 4 }}>
          ...
          <Tooltip title="Atualizar vagas">
            ...
            >
              <RefreshIcon sx={{ animation: loading ? "spin 1.5s linear infinite" : "none", "@keyframes spin": { from: { transform: "rotate(0deg)" }, to: { transform: "rotate(360deg)" } } }} />
            </IconButton>
          </Tooltip>
        </Box>
        ...
        {loading && (
          <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", py: 12 }}>
            <CircularProgress size={44} thickness={4} sx={{ color: "#4F8EF7" }} />
          </Box>
        )}
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3, mb: 6 }}>
            {jobs.map((job) => {
              return (
                <Card
                  key={job.id}
                  onClick={() => router.push(`/usuario/${job.id}`)}
                  sx={{
                    background: "#FFFFFF",
                      ...
                      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2.5 }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
                          ...
                          <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 500 }}>
                            {job.location} {job.neighborhood ? `· Bairro: ${job.neighborhood}` : "· Bairro: Centro"}
                          </Typography>
                        </Box>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
